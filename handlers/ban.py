...
async def ban(message: Message):
    ...
    if len(args) == 0:
        await message.reply(f"{message.from_user.first_name}, this command bans an ip or a range. Type /ban help for more information")
        return
    if args[0] == "help":
        await message.reply("/ban &lt;jail&gt; &lt;ip&gt;")
        return
    if len(args) < 2:
        await message.reply(f"{message.from_user.first_name}, this command bans an ip or a range. Type /ban help for more information")
        return
    if not is_ipv4(args[1]):
        ...
        return
    return_code = os.system(f"fail2ban-client set {args[0]} banip {args[1]}")
